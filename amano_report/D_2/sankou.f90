program burgers
    ...
    integer, parameter :: nx = 200 !x方向の格子数
    real(8), parameter :: nu = 0.2d0 !delta_t / delta_x
    ! real(8), parameter :: alpha = 0.0_8 ! >>> 非粘性!
    ...
    ! データ出力
    write(datfile, fmt='("burgers",i6.3,".dat")') i*step
    ...
    do i = 1, 4
        ...
        ! データ出力
        write(datfile, fmt='("./dataout/burgers",i3.3,".dat")') i*step
        call savefile(x, u, trim(datfile))
    ...
    ! 一次精度風上差分法
    !
    subroutine push_upwind(u, dt, dx, step)
        implicit none
        real(8), intent(inout) :: u(:)
        real(8), intent(in)    :: dt
        real(8), intent(in)    :: dx
        ...
        do n = 1, step
            ...
            ! 境界条件
            u(lbx-1) = u(ubx) - dt/dx * (flux(lbx-1) - flux(ubx))
        end do
